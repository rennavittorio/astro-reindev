    ...
    <div>
          ...
          {allTags.map((tag) => (
            <button
              ...
              className={`text-[10px] font-mono px-2 py-0.5 border transition-colors ${
                activeTags.has(tag) ? "border-white/50 text-white/70" : "border-white/10 text-white/25 hover:border-white/30 hover:text-white/50"
              }`}>
              {tag}
            </button>
          ))}
      ...
      {filtered.length === 0 ? (
        ...
        <ul className="space-y-8">
          {filtered.map((post) => (
            <li key={post._id}>
              <a href={`/blog/${post.slug.current}`} className="group block">
